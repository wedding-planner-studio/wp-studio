...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Input
    ...
    className={cn(
      'flex h-10 w-full rounded-md bg-transparent py-2 text-sm outline-none focus:border-none focus:ring-0',
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ...
    className={cn(
      ...
      'text-gray-900 hover:bg-purple-100 hover:text-purple-900 active:bg-purple-200',
...
// Add a new component for the search input with wrapper
const CommandInputWithWrapper = React.forwardRef<
  React.ElementRef<typeof CommandPrimitive.Input>,
  React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b px-3 sticky top-0 bg-white" data-cmdk-input-wrapper="">
    <Search className="mr-2 h-4 w-4 shrink-0 text-purple-500" />
    <CommandInput ref={ref} className={className} {...props} />
  </div>
));

CommandInputWithWrapper.displayName = 'CommandInputWithWrapper';

export {
  Command,
  CommandDialog,
  CommandInput,
  CommandInputWithWrapper,
  CommandList,
  CommandEmpty,
  CommandGroup,
  CommandItem,
